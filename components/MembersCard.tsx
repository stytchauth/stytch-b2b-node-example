...
const isValidEmail = (emailValue: string) => {
  ...
  const MemberRow = ({ member, currentUser }: Pick<Props, "member" | "currentUser">) => {
    ...
    return (
      <li>
        ...
        <span className="code">{member.email_address}</span> (
        {member.status})&nbsp;
      </li>
    );
  };
    ...
    return (
      <>
        <div className="section">
          <ul>
            {members.map((member) => (
              <MemberRow key={member.member_id} member={member} currentUser={currentUser} />
            ))}
          </ul>
        </div>
        ...
        <div className="section">
          ...
          <form onSubmit={onInviteSubmit} className="row">
            <input
              placeholder={`your-coworker@${organization.email_allowed_domains[0] ?? "example.com"
                }`}
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              type="email"
            />
            <button className="primary" style={{width: 200}} disabled={isDisabled} type="submit">
              Invite
            </button>
          </form>
        </div>
  ...
  const MembersCard = ({organization, members, currentUser}:
    Pick<Props, "organization" | "members" | "currentUser">) => {
    return <div className="card profile-card" >
      <h1>Members</h1>
      <p>You're currently logged in as&nbsp;
        <span className="code">{currentUser.email_address}</span>.
        Below, you'll find a full list of Members who belong to&nbsp;
        {organization.organization_name}:
      </p>
      <MemberList organization={organization} members={members} currentUser={currentUser} />
    </div>
  }
